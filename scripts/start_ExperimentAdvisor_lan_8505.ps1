...
$projectName = "ExperimentAdvisor"
$port = 8505
$ruleName = "ExperimentAdvisor Streamlit 8505 LAN"
...
Write-Host ""
Write-Host "$projectName will start on the local network:" -ForegroundColor Green
foreach ($address in $lanAddresses) {
    Write-Host "  http://$address`:$port" -ForegroundColor Green
}
Write-Host "  http://127.0.0.1`:$port" -ForegroundColor Green
Write-Host ""
Write-Host "Close this window or press Ctrl+C to stop the service." -ForegroundColor Yellow
Write-Host ""
