...
- canonical analysis 现在给出**曲率方向**而不只是特征值大小：`CanonicalAnalysis.eigenvectors`（符号锚定，最大分量恒为正），特征值图的柱子按组成因素命名（"初始装液量↑"、"培养基 pH↑+补料葡萄糖浓度↓"），canonical 表新增「无曲率方向」列（只在岭线/近似平坦时填）。这让"岭线"这个判定从劝退变成可执行：那个方向就是可以按成本/操作难度自由挑点的自由度。同时 `predict_with_confidence_interval` 支持 `n_observations=n`，给出 n 个重复均值的预测区间（默认 `inf` 保持原有的平均响应置信区间）——Round 3 的确认判据要用它，见 ADR-0018 和 `docs/ROUND3_PLAN.md`。
...
| ~~评估是否放宽装液量上限 / 是否测温度~~ | — | **已关闭（ADR-0019）**：250 mL 有挡板瓶装 75 mL 已是 30% 装液率，再高会溅出、换大瓶成本过高 → 上限不放宽；温度因摇床是全箱共享参数（一台设备一个温度，分批会引入批次效应）推迟到项目后期。溶氧这条线移到发酵罐，**但归因必须用配气而非降转速**——降转速同时改 kLa 和剪切。留下两条口径缺口：所有"最优条件"建立在未验证的 30℃ 上，且 75 mL 是设备上限不是实测最优 |
